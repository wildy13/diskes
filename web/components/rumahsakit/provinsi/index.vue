<script setup>
import { useRumahSakitProvinsiStore } from '../../../stores/rumahSakit/provinsi.js';

const store = useRumahSakitProvinsiStore();
...
<template>
    <div class="px-12 h-auto w-full py-8">
        ...
        <RumahsakitProvinsiCreate :show="showCreate" @close="showCreate = false" />
        <RumahsakitProvinsiEdit :show="showEdit" :data="itemData" @close="showEdit = false" />
        <RumahsakitProvinsiRemove :show="showRemove" :data="selected" @close="showRemove = false" />
    </div>
</template>